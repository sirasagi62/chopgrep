...
import { bulkInsertChunks, searchSimilar, close, EMBEDDING_DIM, type CodeChunkRow } from './db'; // Import from db.ts
  ...
  try {
    ...
    if (embeddingArray) {
      ...
      if (embeddingArray.length !== EMBEDDING_DIM) {
        console.warn(`Embedding dimension mismatch: expected ${EMBEDDING_DIM}, got ${embeddingArray.length}. Truncating or padding.`);
        ...
        truncatedOrPadded.set(embeddingArray.slice(0, copyLength));
...
async function indexDirectory(dirPath: string, isJsonOutput: boolean): Promise<void> {
  ...
  try {
    ...
    if (chunksToInsert.length > 0) {
      ...
      if (isJsonOutput) {
        console.log(JSON.stringify({ status: "success", message: `Indexed ${indexedCount} code chunks.`, directory: path.resolve(dirPath) }));
      } else {
        console.log(`Indexed ${indexedCount} code chunks.`);
      }
    } else {
      if (isJsonOutput) {
        console.log(JSON.stringify({ status: "success", message: "No code chunks found to index.", directory: path.resolve(dirPath) }));
      } else {
    ...
    if (isJsonOutput) {
      console.error(JSON.stringify({ status: "error", message: `Error during directory indexing: ${error.message}`, directory: path.resolve(dirPath) }));
    } else {
...
async function query(queryText: string, k: number, isJsonOutput: boolean): Promise<void> {
  ...
  if (results.length > 0) {
    ...
    if (isJsonOutput) {
      console.log(JSON.stringify({ status: "success", query: queryText, k: k, results: output }));
    } else {
    ...
    if (isJsonOutput) {
      console.log(JSON.stringify({ status: "success", query: queryText, k: k, results: [] }));
    } else {
...
async function main() {
  const argv = await yargs(hideBin(process.argv))
    ...
    .command(
      ...
      async (args) => {
        await query(args.queryText as string, args.k as number, args.json as boolean);
        close(); // Close the database connection after command execution
      }
    )
    .option('json', {
